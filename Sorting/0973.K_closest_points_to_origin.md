...
难度: Medium
标签: Array、Heap (PQ)、QuickSelect

![973](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.99u2cbpqaj.webp)
...
class Solution {
    public int[][] kClosest(int[][] points, int k) {
        PriorityQueue<int[]> pq = new PriorityQueue<>(
            (point1, point2) -> Long.compare(distance(point2), distance(point1))
        );
        for (int[] point : points) {
            pq.offer(point);
            if (pq.size() > k) {
