...
![417](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.77ea9h2drv.webp)
...
class Solution {
    ...
    public List<List<Integer>> pacificAtlantic(int[][] heights) {
        ...
        // Pacific: left edge and top edge.
        for (int row = 0; row < m; row++) {
            pacificStarts.add(new int[]{row, 0});
        }

        for (int col = 0; col < n; col++) {
            pacificStarts.add(new int[]{0, col});
        }

        // Atlantic: right edge and bottom edge.
        for (int row = 0; row < m; row++) {
            atlanticStarts.add(new int[]{row, n - 1});
        }

        for (int col = 0; col < n; col++) {
            atlanticStarts.add(new int[]{m - 1, col});
        }

        bfs(pacificStarts, pacific);
        bfs(atlanticStarts, atlantic);

        // Find the intersection of the two reachable regions.
        for (int row = 0; row < m; row++) {
            for (int col = 0; col < n; col++) {
    ...
    private void bfs(List<int[]> starts, boolean[][] ocean) {
        Deque<int[]> queue = new ArrayDeque<>();

        // Add all ocean-border cells to the queue.
        for (int[] start : starts) {
            int row = start[0];
            int col = start[1];

            // Prevent duplicate corner cells.
            if (!ocean[row][col]) {
                ocean[row][col] = true;
        ...
        while (!queue.isEmpty()) {
            ...
            for (int[] dir : dirs) {
                ...
                if (
                    ...
                    // Mark visited when adding it to the queue.
