...
难度: Medium
标签: Linked List、Hashing

![138](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.8okddas606.webp)
...
class Solution {
    public Node copyRandomList(Node head) {
        ...
        Node cur = head;
        while (cur != null) {
            if (!originalToCopy.containsKey(cur)) {
                originalToCopy.put(cur, new Node(cur.val));
            }
            cur = cur.next;
        }
        cur = head;
        while (cur != null) {
            if (cur.next != null) {
                originalToCopy.get(cur).next = originalToCopy.get(cur.next);
            }

            if (cur.random != null) {
                originalToCopy.get(cur).random = originalToCopy.get(cur.random);
            }
