...
![435](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.b9m9y6h4s.webp)
...
class Solution:
    def eraseOverlapIntervals(self, intervals: List[List[int]]) -> int:
        # Handle an empty input for robustness.
        if not intervals:
            return 0
...
class Solution {
    public int eraseOverlapIntervals(int[][] intervals) {
        ...
        for (int i = 1; i < intervals.length; i++) {
            ...
            // Touching intervals are allowed, so only "<" means overlap.
            if (start < prevEnd) {
                removals++;
            } else {
                prevEnd = end;
