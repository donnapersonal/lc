...
难度: Medium
标签: Graph、Dijkstra、Bellman Ford

![787](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.5xbdawva5v.webp)
...
`Bellman-Ford` 本身就是设计`用来在固定步数内找最短路径`的，尤其适合这题 —— 不用建图、无环路径、保证能处理 `K` 约束
...
```java
class Solution {
    public int frindCheapestPrice(int n, int[][] flights, int src, int dst, int k) {
