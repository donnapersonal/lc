...
![743](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.7sny2td5j7.webp)
...
class Solution:
    def networkDelayTime(self, times: List[List[int]], n: int, k: int) -> int:
        ...
        # Add every directed edge:
        # u -> v with travel time w.
        for u, v, w in times:
            graph[u].append((v, w))

        # distance[node] stores the shortest known time
        # from k to that node.
        dist = [float("inf")] * (n + 1)
        dist[k] = 0

        # Min-heap entries are:
        ...
        # Ignore index 0 because nodes are labeled from 1 to n.
        res = max(dist[1:])
...
class Solution {
    public int networkDelayTime(int[][] times, int n, int k) {
        ...
        for (int[] edge : times) {
            ...
            graph.g(u, key -> new ArrayList<>()).add(new int[]{v, w});
        }
        ...
        while (!minHeap.isEmpty()) {
            ...
            for (int[] neiInfo : graph.getOrDefault(node, new ArrayList<>())) {
                int nei = neiInfo[0];
                int weight = neiInfo[1];

                if (!dist.containsKey(nei)) {
                    minHeap.offer(new int[]{time + weight, nei});
                }
            }
        }
...
class Solution {
    public int networkDelayTime(int[][] times, int n, int k) {
        ...
        // Find the largest shortest dist.
