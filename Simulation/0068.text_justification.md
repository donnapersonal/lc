...
![68](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.6t7swkewa1.webp)
...
class Solution {
    public List<String> fullJustify(String[] words, int maxWidth) {
        ...
        for (String word : words) {
            if (!curLine.isEmpty() && lineLen + word.length() + curLine.size() > maxWidth) {
                res.add(justifyLine(curLine, lineLen, maxWidth, false));
                curLine.clear();
                lineLen = 0;
            }
        ...
        res.add(justifyLine(curLine, lineLen, maxWidth, true));
        return res;
    }

    private String justifyLine(List<String> lineWords, int lineLen, int maxWidth, boolean isLastLine) {
        if (lineWords.size() == 1 || isLastLine) {
            String joined = String.join(" ", lineWords);
            return joined + spaces(maxWidth - joined.length());
        }
