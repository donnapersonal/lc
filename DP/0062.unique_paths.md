...
难度: Medium
标签: Matrix、Math、DP、Combinatorics

![62](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.4qs23kti0m.webp)
...
class Solution {
    public int uniquePaths(int m, int n) {
        ...
        for (int i = 1; i < m; i++) {
            for (int j = 1; j < n; j++) {
                // dp[j] before update means paths from top.
                // dp[j - 1] means paths from left.
                dp[j] += dp[j - 1];
            }
        }
