...
![152](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.64el7lsdvv.webp)
...
class Solution {
    public int maxProduct(int[] nums) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            maxNum = Math.max(Math.max(tempMax * cur, cur), tempMin * cur);
            minNum = Math.min(Math.min(tempMin * cur, cur), tempMax * cur);
