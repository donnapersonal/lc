...
![139](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.1ziy0hvoap.webp)
...
class Solution {
    ...
    public boolean wordBreak(String s, List<String> wordDict) {
        ...
        // Use a HashSet for O(1) average word lookup.
        ...
        // memo[start]:
        // null  -> not computed
        // true  -> s[start:] can be segmented
        // false -> s[start:] cannot be segmented
        memo = new Boolean[n];
    ...
    private boolean dfs(String s, int start) {
        ...
        // Only try substrings whose length
        // does not exceed the longest dictionary word.
        int maxEnd = Math.min(n, start + maxLen);
