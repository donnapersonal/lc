...
难度: Hard
标签: Array、Queue、Monotonic Queue、PQ

![239](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.9rk3o3f44r.webp)
...
class Solution {
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // 4. Record the maximum once the window is fully formed
            if (i >= k - 1) {
...
class Solution {
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        for (int i = k; i < n; i++) {
            ...
            // Remove expired elements from the heap top.
            // The current window starts at i - k + 1,
            // so indexes <= i - k are outside the window.
            while (maxHeap.peek()[1] <= i - k) {
                maxHeap.poll();
            }

            // The heap top is the maximum of the current window.
