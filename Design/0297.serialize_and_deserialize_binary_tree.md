...
难度: Hard
标签: Design、Binary Tree、DFS、Level-Order

![297](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.64ej3hrf5o.webp)
...
# Optimized Python Version with Index Pointer
# This avoids pop(0) and keeps the same preorder logic.
class Codec:
    def serialize(self, root):
        if not root:
            return "None"
...
class Codec:
    ...
    def serialize(self, root):
        ...
        def dfs(node: Optional["TreeNode"]) -> None:
            # Record missing children so the exact structure is preserved.
            if node is None:
    ...
    def deserialize(self, data):
        ...
        def dfs() -> Optional["TreeNode"]:
            ...
            # A null marker represents an empty subtree.
            if current == self.NULL:
                return None

            # Create the current root and reconstruct both subtrees.
