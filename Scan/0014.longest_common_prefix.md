...
难度: Easy
标签: Array、String、Scan

![14](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.86uc0ll4a5.webp)
...
class Solution {
    public String longestCommonPrefix(String[] strs) {
        // Defensive check for an empty input array.
        ...
        // Check each character position in the reference string.
        for (int i = 0; i < reference.length(); i++) {
            char expectedChar = reference.charAt(i);

            // Verify that every other string has the same character
            // at the current position.
            for (int j = 1; j < strs.length; j++) {

                // The common prefix ends if the string is too short
                // or if its character does not match.
                if (
                    i >= strs[j].length()
                    || strs[j].charAt(i) != expectedChar
                ) {
        ...
        // Every character in the reference string matched.
        return reference;
    }
}
```
