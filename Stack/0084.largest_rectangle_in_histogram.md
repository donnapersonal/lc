...
![84](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.7ehh3vg648.webp)
...
class Solution {
    public int largestRectangleArea(int[] heights) {
        ...
        // Each entry stores:
        // [earliestStartIndex, height]
        //
        // Heights remain strictly increasing.
        Deque<int[]> stack = new ArrayDeque<>();

        // Process one virtual bar of height 0 at the end.
        for (int currentIndex = 0; currentIndex <= n; currentIndex++) {
            int currentHeight =
                currentIndex == n ? 0 : heights[currentIndex];

            int startIndex = currentIndex;

            // Finalize every taller or equal bar.
            while (
                !stack.isEmpty()
                && stack.peek()[1] >= currentHeight
            ) {
                int[] previousBar = stack.pop();

                int previousStart = previousBar[0];
                int previousHeight = previousBar[1];

                int width = currentIndex - previousStart;
                int area = previousHeight * width;

                maxArea = Math.max(maxArea, area);

                // The current shorter bar inherits the earlier
                // valid start position.
                startIndex = previousStart;
            }

            stack.push(new int[] {startIndex, currentHeight});
        }
