...
难度: Medium
标签: Binary Tree、Recursion

![105](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.64ej3hrf5o.webp)
...
class Solution {
    public TreeNode buildTree(int[] preorder, int[] inorder) {
        ...
        // The stack stores the current path of nodes being processed.
        Deque<TreeNode> stack = new ArrayDeque<>();
        stack.push(root);
        // Points to the next node that should be matched in inorder.
        int inorderIndex = 0;
        for (int i = 1; i < preorder.length; i++) {
            int preorderValue = preorder[i];
            // The stack top is the current possible parent node.
            TreeNode node = stack.peek();
            // If the stack-top node has not matched the current inorder
            // value, we are still building its left subtree.
            if (node.val != inorder[inorderIndex]) {
                node.left = new TreeNode(preorderValue);
                stack.push(node.left);
            } else {
                // Pop all nodes whose left subtrees have been completed.
                // The last popped node is the parent of the next right child.
                while (!stack.isEmpty() && inorderIndex < inorder.length && stack.peek().val == inorder[inorderIndex]) {
                    node = stack.pop();
                    inorderIndex++;
                }

                // The current preorder value becomes the right child
                // of the last popped node.
