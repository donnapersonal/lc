...
![example](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.491ge6vp0h.webp)
...
class Solution {
    public int numTrees(int n) {
        ...
        // Empty tree counts as one valid structure.
        dp[0] = 1;

        // i means the number of nodes.
        for (int i = 1; i <= n; i++) {
            // j means choosing the j-th node as the root.
            for (int j = 1; j <= i; j++) {
                int left = j - 1;
                int right = i - j;
