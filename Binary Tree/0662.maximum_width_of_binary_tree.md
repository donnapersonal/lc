...
class Solution {
    public int widthOfBinaryTree(TreeNode root) {
        ...
        while (!queue.isEmpty()) {
            int size = queue.size();
            long firstIndex = queue.peekFirst().index;
            long lastIndex = 0;

            for (int i = 0; i < size; i++) {
                Pair current = queue.pollFirst();
                TreeNode node = current.node;
                long index = current.index - firstIndex;   // normalize
                ...
                if (node.left != null) {
                    queue.offerLast(new Pair(node.left, 2 * index));
                }
                if (node.right != null) {
                    queue.offerLast(new Pair(node.right, 2 * index + 1));
                }
            }
