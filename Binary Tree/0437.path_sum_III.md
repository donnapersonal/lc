...
class Solution {
    ...
    private int dfs(TreeNode node, long curSum, int targetSum, Map<Long, Integer> prefixSums) {
        if (node == null) {
            return 0;
        }
        ...
        int count = prefixSums.getOrDefault(curSum - targetSum, 0);

        prefixSums.put(curSum, prefixSums.getOrDefault(curSum, 0) + 1);

        count += dfs(node.left, curSum, targetSum, prefixSums);
        count += dfs(node.right, curSum, targetSum, prefixSums);
