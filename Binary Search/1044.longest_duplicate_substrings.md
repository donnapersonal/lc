...
class Solution {
    ...
    private final long MOD1 = 1_000_000_007L;
    private final long MOD2 = 1_000_000_009L;
    private final long BASE = 256L;
    ...
    private int check(int len) {
        ...
        seen.computeIfAbsent(key, x -> new ArrayList<>()).add(0);

        for (int i = len; i < n; i++) {
            hash1 = (
                hash1 * BASE
                - s.charAt(i - len) * power1[len] % MOD1
                + s.charAt(i)
            ) % MOD1;
            ...
            hash2 = (
                hash2 * BASE
                - s.charAt(i - len) * power2[len] % MOD2
                + s.charAt(i)
            ) % MOD2;
            ...
            if (seen.containsKey(key)) {
                for (int prevStart : seen.get(key)) {
