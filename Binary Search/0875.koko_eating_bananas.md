...
![74](https://github.com/donnapersonal/picx-images-hosting/raw/master/image.2dpedm7ywp.webp)
...
class Solution:
    ...
    def canFinish(self, piles: List[int], h: int, mid: int) -> bool:
        hours = 0
        for pile in piles:
            # hours += ceil(pile / mid) 先做除法，可能变成浮点数
            # 这是因为它是纯整数运算，更稳，也更能体现你对“整数上取整”的理解
            # 它表示：pile / k 向上取整 即 ceil(pile / k)
            # - 1 是因为：不能整除时，比如 10 和 3
            # 正常除法：10 / 3 = 3.333... ceil(...) = 4
            # 普通整除：10 // 3 = 3，这时少了 1，因为整除默认向下
            # 解决：只要有余数，就想办法让它“多出来一整份”，这样整除后就会自动多 1，所以加上 3 - 1，于是结果就补上了那次向上取整
